...
private:
  ...
  double TP_maxR;      // save TPs with |dxy| < maxR
  ...
  // Gen particle info
  std::vector<float>* m_genp_pt;        // gen particle pt
  std::vector<float>* m_genp_eta;        // gen particle eta
  std::vector<float>* m_genp_phi;        // gen particle phi
  std::vector<int>*   m_genp_pdgid;        // gen particle pdgid
  ...
  std::vector<int>*   m_tp_nstub_layer;
  std::vector<int>*   m_tp_nstub_layer_g;
  std::vector<int>*   m_tp_eventid;
  std::vector<int>*   m_tp_vtxid;
  ...
  std::vector<float>* m_allstub_module_x;               // module x coordinate
  std::vector<float>* m_allstub_module_y;               // module y coordinate
  std::vector<float>* m_allstub_module_z;               // module z coordinate
  ...
  config(iConfig)
{
  ...
  TP_maxR         = iConfig.getParameter< double >("TP_maxR");
  ...
  GenParticleToken_ = consumes<vector<reco::GenParticle>>(GenParticleInputTag);
}
...
void L1TrackNtupleMaker::beginJob()
{
  ...
  // initilize
  m_genp_pt     = new std::vector<float>;
  m_genp_eta    = new std::vector<float>;
  m_genp_phi    = new std::vector<float>;
  m_genp_pdgid    = new std::vector<int>;
  ...
  m_tp_nstub_layer  = new std::vector<int>;
  m_tp_nstub_layer_g= new std::vector<int>;
  ...
  eventTree->Branch("genp_pt",     &m_genp_pt);
  eventTree->Branch("genp_eta",    &m_genp_eta);
  eventTree->Branch("genp_phi",    &m_genp_phi);
  eventTree->Branch("genp_pdgid",    &m_genp_pdgid);
  ...
  eventTree->Branch("tp_nstub_layer", &m_tp_nstub_layer);
  eventTree->Branch("tp_nstub_layer_g",&m_tp_nstub_layer_g);
...
void L1TrackNtupleMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  m_tp_nstub->clear();
  m_tp_nstub_layer->clear();
  m_tp_nstub_layer_g->clear();
  m_tp_eventid->clear();
  m_tp_charge->clear();
  m_tp_injet->clear();
  ...
  iEvent.getByToken(GenParticleToken_, pGenParticles) ;
  reco::GenParticleCollection genParticles(pGenParticles->begin(),pGenParticles->end()) ;

  for(reco::GenParticleCollection::const_iterator mc_iter = genParticles.begin() ; mc_iter!=genParticles.end() ; ++mc_iter){
    m_genp_pt->push_back(mc_iter->pt());
    m_genp_eta->push_back(mc_iter->eta());
    m_genp_phi->push_back(mc_iter->phi());
    m_genp_pdgid->push_back(mc_iter->pdgId());
  }
  ...
  if (SaveStubs) {
	...
	m_allstub_trigBend->push_back(trigBend);
        ...
        MeasurementPoint mp0 = tempStubPtr->getClusterRef(0)->findAverageLocalCoordinates();
        ...
        m_allstub_chip->push_back(int(tempStubPtr->getTriggerPosition()/chipSize));
...
std::cout<<layer<<" ladder "<<ladder<<" module "<<module<<" z "<<theGeomDet->surface().position().z()<<std::endl;
	...
	if (my_tp.isNull() == false) {
	  ...
	  if (tmp_eventid > 0){  // this means stub from pileup track
            myTP_pdgid = -555;
            myTP_pt = -555;
            myTP_eta = -555;
            myTP_phi = -555;
  ...
  for (iterTP = TrackingParticleHandle->begin(); iterTP != TrackingParticleHandle->end(); ++iterTP) {
    ...
    const TrackingVertex& r=( *iterTP->parentVertex());
    if(r.eventId().event()!=0) continue;
    if (abs(iterTP->pdgId()) == 13  || abs(iterTP->pdgId()) == 11 || abs(iterTP->pdgId()) == 211){
      ...
      for (unsigned int k=0; k<MCTruthTTClusterHandle->findTTClusterRefs(tp_ptr).size(); ++k) {
        if(MCTruthTTClusterHandle->isUnknown(MCTruthTTClusterHandle->findTTClusterRefs(tp_ptr).at(k))) continue;
      ...
      for (unsigned int k=0; k<MCTruthTTStubHandle->findTTStubRefs(tp_ptr).size(); ++k) {
        if(!MCTruthTTStubHandle->isGenuine(MCTruthTTStubHandle->findTTStubRefs(tp_ptr).at(k))) continue;
        DetId stubdetid = MCTruthTTStubHandle->findTTStubRefs(tp_ptr).at(k)->getDetId();
        if ( stubdetid.subdetId()==StripSubdetector::TOB ) {
          stub_B[static_cast<int>(tTopo->layer(stubdetid)) - 1]++; 
        }
        else if ( stubdetid.subdetId()==StripSubdetector::TID ) {
          stub_E[static_cast<int>(tTopo->layer(stubdetid)) - 1]++;
        }
      }
  ...
  m_tp_N = (int) TrackingParticleHandle->size();
  for (iterTP = TrackingParticleHandle->begin(); iterTP != TrackingParticleHandle->end(); ++iterTP) {
...
//    if (fabs(tmp_tp_rp) > TP_maxR) continue;
...
//	continue;
	...
	*/
    ...
    m_tp_nstub_layer->push_back(nStubLayerTP);
    m_tp_nstub_layer_g->push_back(nStubLayerTP_g);
